...
function App (rootElement) {
  ...
  function useVideoAdapter(state, videoAdapter) {
    ...
    // Set remote video's initial size to window
    remoteVideo.setDimensions(state.remoteVideo, wr.getWinSize())
    // When our window resizes, change the remoteVideo to fit.
    wr.winResize.on(function(dimensions) {
      remoteVideo.setDimensions(state.remoteVideo, dimensions)
    })
