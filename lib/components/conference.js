  ...
  function onVideosLoaded() {
    ...
    function onIncomingPeer (stream) {
      ...
      if (videoElements.remote.childNodes.length > 0) {
        ...
        var node = videoElements.remote.childNodes[0]
      ...
      peer.on('connected', function () {
        ...
        videoElements.remote.style.width =
        videoElements.remote.style.height = "100%";

        // OT.updateViews only exists in OpenTok for cordova
        if (typeof OT !== 'undefined' && OT.updateViews) {
          OT.updateViews()
        }
      })
